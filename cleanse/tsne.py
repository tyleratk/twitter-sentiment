...
# pos_tweets = gb.get_group('pos').sample(100000).text.values
# neg_tweets = gb.get_group('neg').sample(100000).text.values
...
for topic_idx, topic in enumerate(nmf.components_):
    dct[topic_idx] = " ".join([tfidf_feature_names[i] for i in topic.argsort()[:-n_top_words - 1:-1]])    


# ------------------ visualization ----------------------------------------------
    # tsne = TSNE(n_components=2, random_state=0)
    # # train_X_tsne = tsne.fit_transform(tfidf, )
    # 
